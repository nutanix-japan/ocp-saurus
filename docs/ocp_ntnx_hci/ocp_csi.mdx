...
5.  Export the OCP cluster's KUBECONFIG file to environment so we can perform `oc` commands
    ...
    ``` bash title="On NCM deployed OCP Clusters"
    export KUBECONFIG=~/openshift/auth/kubeconfig
    ```
    ``` bash title="On IPI deployed OCP Clusters"
...
9.  List your StorageClass
    ...
    nutanix-volume(default)   csi.nutanix.com   Delete          Immediate           true                   4m
    ```
...
In this section we will install StorageClass with Nutanix Files target.
...
1. Use the following yaml file to create a storage class with Nutanix files as destination
   ...
   ```bash 
   cat <<EOF > storageclass_files.yaml 
   apiVersion: storage.k8s.io/v1
   kind: StorageClass
   metadata:
     name: nutanix-files
   ...
   parameters:
     nfsPath: /xyz_nfs                    ## << use the file share you created in the previous section
     nfsServer: bootcampfs.ntnxlab.local  ## << use your file server name - make sure name resolution is working
...
2. Edit the file to make configure your file share and file server 
   ...
   ```bash
   vi storageclass_files.yaml
   ```

3. Create the storage class 

   ```bash
   oc apply -f storageclass_files.yaml
   ```
   ```buttonless title="Output"
   storageclass.storage.k8s.io/nutanix-files created
   ```
4. List your storage classes
   ...
   ```buttonless title="Output"
   NAME                       PROVISIONER       RECLAIMPOLICY   VOLUMEBINDINGMODE   ALLOWVOLUMEEXPANSION   AGE
   nutanix-files              com.nutanix.csi   Delete          Immediate           false                  67s
   nutanix-volume (default)   csi.nutanix.com   Delete          Immediate           true                   17m   
   ```