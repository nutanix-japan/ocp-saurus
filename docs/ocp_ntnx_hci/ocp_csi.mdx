...
``Static`` mode Files stoage class is also available, use the following manifest to create this. The file share needs to manually created before the storage class can be configured.
