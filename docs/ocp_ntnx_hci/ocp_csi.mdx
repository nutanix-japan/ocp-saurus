...
9.  In the Operator install wizard choose the following:
    ...
    -   **Operator recommended Namespace** - ``ntnx-system`` (or ``openshift-cluster-csi-drivers`` namespace)
...
4. Create a new ``NFSv4`` share by filling in the following details
   ...
   Make sure your Files share is NFSv4. 
...
1. Use the following yaml file to create a storage class with Nutanix files as destination
   ...
   parameters:
     ...
     nfsServer: bootcamp-fs.ntnxlab.local ## << use the file server 
...
4. List your storage classes
   ...
   ```buttonless title="Output"
   NAME                      PROVISIONER       RECLAIMPOLICY   VOLUMEBINDINGMODE   ALLOWVOLUMEEXPANSION   AGE
   nutanix-volume(default)   csi.nutanix.com   Delete          Immediate           true                   8m
   nutanix-files             csi.nutanix.com   Delete          Immediate           true                   4m      
   ```