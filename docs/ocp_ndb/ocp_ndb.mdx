...
10. It will take about 20 minutes to provision the database server, once done, check the status in OCP
...
3. You can analyse the logs to make sure the job has completed without any issues
   ...
   ```bash title="Analyse the pod logs"
   ...
   You would have to delete your job, change references in **app_variables** config map, and re-run it
