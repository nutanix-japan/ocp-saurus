...
1. Change to the downloaded NDB opertor git repo folder
   
   ```bash
   cd /root/ndb-operator
   ```
...
3. Create a secret with credentials (password and ssh public key) for the new Postgres VM that you will be provisioning.
   ...
   SSH key is a requirement. This must be included to authenticate to your NDB deployed VM. ``Database`` resource will fail to deploy if not included.
