...
4. Create a Database resource by using the following manifest
   ...
   spec:
     ...
     ndb:
       ...
       # NDB cluster ID can be fetched from the GET /clusters endpoint
       clusterId: "b46cdb17-4731-47ac-a986-839ca4506909"    # << NDB cluster ID from previous step
       # Credentials secret name for NDB installation
       # data: username, password, 
       # stringData: ca_certificate
       credentialSecret: your-ndb-secret
       # The NDB Server
       server: https://[NDB IP]:8443/era/v0.9               # << Change this to use your NDB server IP
       # Set to true to skip SSL verification, default: false.
       skipCertificateVerification: true
      # Database instance specific details (that is to be provisioned)
      databaseInstance:
        # The database instance name on NDB
        databaseInstanceName: "pgserver"                  # << Create a new Postgres server
        # Names of the databases on that instance
        databaseNames:
        - predictiondb                                  # <<  Create a new database 
        # Credentials secret name for NDB installation
